...
function parseNotionRichTextToHTML(richText: RichTextType) {
    ...
    return <p>{nodes.map((n, idx) => <Fragment key={idx}>{n}</Fragment>)}</p>
}
...
export function ExperienceCard({experience: {label, company, companyLogo, to, from, summary}}: Props) {
    return (
        <li className='snap-center px-4 w-full lg:w-1/3 flex-shrink-0 relative self-start snap-always mx-auto'>
            ...
            <article
                className="block p-6 rounded-xl shadow-lg w-full bg-gray-100 mt-6 mb-10">
                ...
                <header className="mb-4">
                    ...
                    <p className='font-light uppercase text-xs tracking-wide text-indigo-600'>{company} &bull; {formatExperienceDate(from)} à {formatExperienceDate(to)}</p>
                </header>
                <article
